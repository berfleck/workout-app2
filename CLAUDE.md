...
tools/
  ...
  gerar_curadoria_video.py     — Curadoria de YouTube: HTML interativo, abre browser ao rodar
  merge_youtube_catalogo.py    — Merge de tools/escolhas_youtube.json → midia_exercicios.json
  escolhas_midia_raw.json      — Escolhas de imagem já curadas (Trilha 1)
...
**Teto de complexidade da UI no CSP (`max_complexidade`)** — fix 2026-06-18: o slider "complexidade máxima" tinha ficado morto na migração pro motor CSP (Frentes C/E.1). O valor era lido no form e gravado na cfg, mas nunca chegava a `gerar_rotina_csp`/`gerar_treino_csp` — o único corte de complexidade era o H-P1 por nível do aluno (`filtrar_pool_por_nivel`, teto nível 1→2, 2→3, 3→5). Agora `max_complexidade: int = 5` é threadado por `_construir_modelo` → `_resolver_legacy`/`_resolver_com_variedade` → `gerar_rotina_csp` → `gerar_treino_csp` e aplicado como filtro hard de pool junto do H-P1 (teto efetivo = `min(teto_nível, max_complexidade)`), antes do solver. Default 5 = no-op (preserva harness/pytest byte-a-byte). Travados bypassam por construção (pool do slot travado é `[ex_travado]`, independente de `banco_nivel` — decisão do personal supera a regra, igual Fatia 4.D). Wirado nos 2 call-sites do app: `/gerar` passa `max_cx`; `/regerar` passa `cfg_r["max_complexidade"]`.
...
## Página do aluno (Mídia)

Página HTML standalone, mobile-first, marca BF Treinamento. Aluno acessa via **link permanente
servido pelo próprio app** (`/p/<slug>`, no Railway). Botão "Ver" por exercício — fallback:
**YouTube embedded > imagem animada > nada**.

### Arquitetura (link vivo por aluno — desde 2026-06-14)
- `pagina_aluno.py`: `gerar_html(rotina, catalogo)` / `gerar_html_de_sessoes(sessoes, aluno_nome)`.
  `_ex_html` lê `yt = ex.get("yt") or (m.get("youtube") if m else None)` — catálogo tem prioridade,
  `ex["yt"]` funciona como override por exercício. Função pura → HTML self-contained.
- `midia_exercicios.json`: catálogo `nome→{media_id, media_name, img0, img1, youtube?}`.
  Join por nome do exercício. **NÃO tocar no dataclass `Exercicio` nem em gerador_treino.py.**
- **Servida pelo Flask** (não mais Cloudflare): rota pública `GET /p/<slug>` em `app_flask.py`.
  Slug inadivinhável **por aluno** = `sha256(PUBLISH_SALT + aluno_id)[:16]` (helper `_slug_aluno`,
  lookup `_aluno_por_slug`). `PUBLISH_SALT` vem de env (OBRIGATÓRIO em produção — fallback é só dev).
  A página renderiza a **rotina ATIVA atual** do aluno → **link vivo**: atualizou a rotina, a página
  atualiza sozinha, sem reenviar. Não expõe rascunhos.
...
```
python tools/gerar_curadoria_video.py   # gera HTML e abre browser
# → colar links (watch?v=, youtu.be/, /shorts/), baixar JSON
# → salvar como tools/escolhas_youtube.json
python tools/merge_youtube_catalogo.py  # merge no catálogo
# → re-publicar rotina no HUB
```

**⚠️ Encoding:** o browser exporta UTF-8, mas pode chegar double-encoded ao colar no chat
(acentos aparecem como `Ã§Ã£o`). Corrigir antes do merge:
```python
corrigido = {k.encode('latin-1').decode('utf-8'): v for k, v in raw.items()}
```

### Estado atual (2026-06-14)
- Trilha 1 (imagens): ✅ 97/148 exercícios com imagem (free-exercise-db, domínio público)
- Trilha 3A (YouTube): 🔄 81/148 links curados — Bernardo continua quando puder
- Trilha 3B (vídeos próprios): pendente — estrutura de fallback já pronta em `pagina_aluno.py`
...
- Trilha 3A: ~67 links YouTube restantes para completar o catálogo
- Trilha 3B: vídeos próprios (ffmpeg → MP4 9:16 mudo em loop)
