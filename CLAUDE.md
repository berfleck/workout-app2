...
Página HTML standalone, mobile-first, marca BF Treinamento. Aluno acessa via link público
(Cloudflare Pages). Botão "Ver" por exercício — fallback: **YouTube embedded > imagem animada > nada**.
...
- `pagina_aluno.py`: `gerar_html(rotina, catalogo)` / `gerar_html_de_sessoes(sessoes, aluno_nome)`.
  ...
  `ex["yt"]` funciona como override por exercício.
...
- `publicador.py`: `publicar_rotina(rotina_id)→url`. Slug = sha256(salt+rotina_id[:14]).
  Re-publicar = mesma URL. Credenciais em `cloudflare_config.json` (gitignorado).
- Rota `POST /hub/rotina/<aluno_id>/publicar` em `app_flask.py`. Botão em `_rotina_hub.html`
  (só quando `rotina and not eh_rascunho`). Partial `_publicar_link.html` (link + Copiar + Abrir).
- Páginas são **snapshots estáticos** — exigem re-publicar após atualizar o catálogo.
